...
from agentgate.context import get_context
from agentgate.engine import PolicyEngine
from agentgate.engine import register_tools as _engine_register_tools
from agentgate.models import ScopePolicy, ToolCall, Verdict, VerdictType
...
def protect_all(
    *,
    ...
    Args:
        ...
        judge_model: Model to use for Tier 2 LLM-as-judge evaluation.
        judge_api_key: API key for Tier 2 judge. Falls back to AGENTGATE_JUDGE_API_KEY / OPENAI_API_KEY.
        judge_base_url: Base URL for an OpenAI-compatible judge endpoint.
    ...
    openai_client = _create_tier2_client(judge_api_key, judge_base_url)

    _engine = PolicyEngine(
        openai_client=openai_client,
        tier2_model=judge_model,
...
def _handle_verdict(verdict: Verdict, tool_call: ToolCall) -> None:
    ...
    from agentgate.logger import log_action_fire_and_forget
...
def _create_tier2_client(
    api_key: str | None = None,
    base_url: str | None = None,
) -> Any:
    """Create a SEPARATE OpenAI-compatible client for Tier 2 LLM-as-judge.

    This is intentionally a different client instance from whatever the agent uses.
    Supports any OpenAI-compatible endpoint (Anthropic, Together AI, Ollama, etc.)
    via base_url / AGENTGATE_JUDGE_BASE_URL.
    """
    key = api_key or os.environ.get("AGENTGATE_JUDGE_API_KEY") or os.environ.get("OPENAI_API_KEY")
    url = base_url or os.environ.get("AGENTGATE_JUDGE_BASE_URL")
    if not key:
        logger.warning(
            "No API key found for Tier 2 judge. "
            "Set OPENAI_API_KEY or AGENTGATE_JUDGE_API_KEY in your environment. "
            "Tier 2 (LLM-as-judge) will be disabled until a key is provided."
        )
        return None
    try:
        ...
        logger.warning("openai package not installed — Tier 2 disabled.")
        return None
...
def _auto_patch() -> list[str]:
    ...
    try:
        from agentgate.interceptors.langchain import patch_langchain
        if patch_langchain():
    ...
    try:
        from agentgate.interceptors.openai import patch_openai
        if patch_openai():
...
def _init_audit_logger() -> None:
    ...
    try:
        from agentgate.logger import AuditLogger
